...
import numpy as np
from ngsphymod.Coverage import NGSPhyDistributionParser as ngsphydistro
...
class Settings:
        ...
        if os.path.exists("{0}/{1}/{2}/".format(self.path,self.projectName,self.outputFolderName)):
            ...
            for item in listdir:
                if self.outputFolderName in item:
                    counter+=1
            if not counter == 0:
                self.outputFolderName="output_{0}".format(counter+1)
        ...
        if (self.parser.has_section("data")):
            ...
            if self.originMode==3:
                ...
                if (self.parser.has_option("data","reference_sequence_file")):
                    self.readCountsReferenceFilePathSequenceFilePath=os.path.abspath(self.parser.get("data","reference_sequence_file").strip())
                    filesOk=(os.path.exists(self.readCountsReferenceFilePathSequenceFilePath) and os.path.isfile(self.readCountsReferenceFilePathSequenceFilePath))
                    if not filesOk:
                        parserMessageWrong+="\n\t{0}{1}{2}\n\t{3}\n\t{4}".format(
                            "[data] section: Origin mode (",self.originMode,") selected but invalid option.",\
    ...
    def checkSectionCoverage(self,parserMessageCorrect,parserMessageWrong):
        ...
        if(self.parser.has_section("coverage")):
            ...
            # INDIVIDUAL -------------------------------------------------------
            if (self.parser.has_option("coverage","genomic_stochasticity")):
                value=self.parser.get("coverage","genomic_stochasticity")
                self.parser.set("coverage","genomic_stochasticity",value.lower())
                self.genomicrnd=ngsphydistro(self.parser.get("coverage","genomic_stochasticity"), False)
                check,mess=self.genomicrnd.validate()
                if not (check):
                    parserMessageWrong+=mess
                    return check,parserMessageWrong
            # INDIVIDUAL -------------------------------------------------------
