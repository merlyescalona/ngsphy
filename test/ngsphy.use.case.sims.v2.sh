...
################################################################################
# Mark duplicates for relaxed mapping
#-------------------------------------------------------------------------------
summaryRelaxedFile="$CURRENT_DIR/${CASE_NAME}/files/duplicates.relaxed.summary.txt"
rm $summaryRelaxedFile
for bamFile in $(find ${CURRENT_DIR}/${CASE_NAME}/mappings-relaxed -type f | grep sorted.bam$); do
    coverageFolder=$(basename $(dirname $(dirname $bamFile)))
    outputDIR=$(dirname $bamFile)
    values=($(basename $bamFile | tr "_" " " | tr "." " " ))
    indID=${values[-4]}
    ...
    header=$(head -7 $metricsOutput | tail -n+7)
    summaryInfo=$(head -8 $metricsOutput | tail -n+8)
    if [[ ! -f $summaryRelaxedFile ]]; then
        echo -e "COVERAGE\tREPLICATE\tINDIVIDUAL_ID\tNUM_MAPPED_READS_SAMTOOLS\tNUM_RECORDS_SAMTOOLS\tNUM_N_READS\t$header" > $summaryRelaxedFile
    fi
    numReads=$(samtools view -c $bamFile)
    numRecords=$(samtools view $bamFile | wc -l)
    numMappedReads=$(samtools view -F 0x4 $bamFile | cut -f 1 | sort | uniq | wc -l)
    echo -e "$coverageFolder\t$repID\t$indID\t$numMappedReads\t$numRecords\t$numReads\t$summaryInfo" >> $summaryRelaxedFile
    tail -n+11 $metricsOutput > $histogramOutput
    samtools index $dedupOutput
done
...
# ------------------------------------------------------------------------------
gacmsa  -i $CURRENT_DIR/${CASE_NAME}/${SIMPHY_PROJECT_NAME}/1/data_1_TRUE.fasta \
